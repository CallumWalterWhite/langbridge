...
@pytest.mark.anyio
async def test_dispatch_service_routes_dataset_jobs_to_edge_queue_for_customer_runtime() -> None:
    tenant_id = uuid.uuid4()
    runtime_id = uuid.uuid4()
    message_service = _FakeMessageService()
    edge_gateway = _FakeEdgeTaskGatewayService()
    service = TaskDispatchService(
        ...
        runtime_registry_service=_FakeRuntimeRegistryService(runtime_id),
        edge_task_gateway_service=edge_gateway,
        request_context_provider=_FakeRequestContextProvider(correlation_id="corr-dataset"),
    )

    mode = await service.dispatch_job_message(
        tenant_id=tenant_id,
        payload=DatasetJobRequestMessage(
            job_id=uuid.uuid4(),
    ...
    assert queued_tenant == tenant_id
    assert queued_runtime == runtime_id
    assert envelope.headers.correlation_id == "corr-dataset"
