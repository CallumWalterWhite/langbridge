...
from langbridge.runtime.persistence.db.messages import OutboxMessage, MessageStatus
...
class MessageRepository(AsyncBaseRepository[OutboxMessage]):
    ...
    async def get_pending_messages(self, correlation_id: str) -> List[OutboxMessage]:
        query = select(OutboxMessage).where(
            OutboxMessage.correlation_id == correlation_id,
            OutboxMessage.status == MessageStatus.not_sent,
        )
        result = await self._session.execute(query)
        return result.scalars().all()
    
    async def get_all_pending_messages(self, timestamp_marker: datetime = None) -> List[OutboxMessage]:
        query = select(OutboxMessage).where(OutboxMessage.status == MessageStatus.not_sent)
        if timestamp_marker:
            query = query.where(OutboxMessage.created_at <= timestamp_marker)
        result = await self._session.execute(query)
        return result.scalars().all()
    
    async def mark_message_as_sent(self, message_id: str) -> None:
        query = select(OutboxMessage).where(OutboxMessage.id == message_id)
        result = await self._session.execute(query)
    ...
    async def mark_messages_as_sent(self, message_ids: List[str]) -> None:
        query = select(OutboxMessage).where(OutboxMessage.id.in_(message_ids))
        result = await self._session.execute(query)
