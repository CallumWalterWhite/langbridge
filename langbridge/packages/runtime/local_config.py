...
class _InMemoryDatasetRepository:
    ...
    async def get_by_ids(self, dataset_ids) -> list[DatasetRecord]:
...
class _InMemoryDatasetColumnRepository:
    def __init__(self, columns_by_dataset: dict[uuid.UUID, list[DatasetColumnRecord]]) -> None:
...
class _InMemoryAgentRepository:
    ...
    async def get_by_id(self, id_: object) -> AgentDefinition | None:
...
class _InMemoryThreadRepository:
    ...
    def add(self, instance: Thread) -> Thread:
        self._threads[instance.id] = instance
        return instance

    async def get_by_id(self, id_: object) -> Thread | None:
...
class _InMemoryThreadMessageRepository:
    ...
    def add(self, instance: ThreadMessage) -> ThreadMessage:
...
class _InMemoryConversationMemoryRepository:
    def __init__(self) -> None:
        self._items: list[ConversationMemoryItem] = []
...
class _InMemorySemanticModelStore:
    def __init__(self, models: dict[uuid.UUID, SemanticModelRecordResponse]) -> None:
    ...
    async def ask_agent(
        ...
        *,
        ...
        thread = Thread(
            ...
            metadata_json={"runtime_mode": "local_config"},
            created_at=timestamp,
            updated_at=timestamp,
        )
...
class ConfiguredLocalRuntimeHostFactory:
        ...
        *,
        ...
        for dataset in datasets.values():
            record = DatasetRecord(
                ...
                relation_identity_json=dict(dataset.relation_identity or {}),
                execution_capabilities_json=dict(dataset.execution_capabilities or {}),
                referenced_dataset_ids_json=[str(item) for item in (dataset.referenced_dataset_ids or [])],
                federated_plan_json=dict(dataset.federated_plan or {}) if dataset.federated_plan else None,
                file_config_json=dict(dataset.file_config or {}) if dataset.file_config else None,
            ...
            policy_record = DatasetPolicyRecord(
                ...
                redaction_rules_json=dict(policy.redaction_rules),
                row_filters_json=list(policy.row_filters),
    ...
    def _build_semantic_model_store_records(
        *,
        semantic_models: dict[str, LocalRuntimeSemanticModelRecord],
        context: RuntimeContext,
    ) -> dict[uuid.UUID, SemanticModelRecordResponse]:
        records: dict[uuid.UUID, SemanticModelRecordResponse] = {}
        now = datetime.now(timezone.utc)
        for semantic_model in semantic_models.values():
            records[semantic_model.id] = SemanticModelRecordResponse(
