...
def build_unified_semantic_model(
    *,
    ...
    Returns the unified model and a mapping of dataset_key -> source connector_id.
    """
    ...
    for source in source_models:
        ...
        for dataset_key in source.model.datasets.keys():
            if dataset_key in dataset_connector_map:
                raise ValueError(f"Duplicate dataset key '{dataset_key}' detected while building unified model.")
            if source.connector_id is not None:
                dataset_connector_map[dataset_key] = source.connector_id
    ...
    return load_semantic_model(unified_payload), dataset_connector_map
...
def apply_tenant_aware_context(
    ...
    *,
    ...
    """
    Produce a tenant-aware semantic model by assigning per-dataset catalogs.

    Catalog naming strategy is deterministic: `org_<org12>__src_<connector12>`.
    Existing explicit dataset catalogs are preserved.
    """
