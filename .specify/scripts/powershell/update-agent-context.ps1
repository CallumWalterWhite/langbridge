...
function Update-CodexProjectContext {
    $guidanceFile = Join-Path $REPO_ROOT '.specify/memory/project-guidance.md'
    Write-Info 'Codex uses the repo-owned AGENTS.md plus .specify memory files for project context.'
    Write-Info 'Skipping AGENTS.md mutation to preserve repository instructions.'
    if (-not (Test-Path $guidanceFile)) {
        Write-WarningMsg "Project guidance file not found at $guidanceFile"
    }
    return $true
}
...
function Update-SpecificAgent {
    ...
    switch ($Type) {
        ...
        'codex'    { Update-CodexProjectContext }
...
function Update-AllExistingAgents {
    ...
    if (Test-Path $AGENTS_FILE)   { if (-not (Update-CodexProjectContext)) { $ok = $false }; $found = $true }
